...
import { adminActivityTitle, compareRegistrationDate, formatAuditDate, leadStatusColor } from '@/lib/prospectPresentation';
    ...
    <section className="card overflow-hidden">
      <div className="border-b border-zinc-200 p-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <FilterSelect label="Trier par" value={sortBy} onChange={(value) => setSortBy(value as typeof sortBy)}><option value="date_desc">Date d’inscription : plus récent</option><option value="date_asc">Date d’inscription : plus ancien</option>{profile?.is_admin && <option value="closer">Closer : A → Z</option>}<option value="crm">Statut CRM</option><option value="ecom">Statut E-com</option><option value="live">Prêt à payer</option></FilterSelect>
        </div>
...
function LeadCard({ lead, selected, onToggle, onContact, onWhatsApp, onCall, onStatus, onHistory, onDelete, onAssign }: LeadActions) {
  ...
  return <article className="space-y-3 p-4">
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1"><div className="break-words font-semibold text-zinc-950">{lead.full_name}</div><a className="mt-0.5 inline-block text-sm font-medium text-aurel-teal" href={whatsappUrl(lead.phone_normalized)} target="_blank" rel="noreferrer" onClick={onWhatsApp}>{lead.phone_raw}</a><div className="break-all text-xs text-zinc-500">{lead.email}</div><div className="mt-1 text-xs text-zinc-500">Inscrit le {formatDateTime(lead.created_at)}</div></div>
