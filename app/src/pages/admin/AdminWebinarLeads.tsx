  ...
  return <div className="space-y-6">
    ...
    <section className="card overflow-hidden">
      <div className="flex flex-wrap gap-3 border-b border-zinc-200 p-4">
        <label className="relative w-full min-w-0 flex-1 sm:min-w-64"><Search className="absolute left-3 top-3.5 h-4 w-4 text-zinc-400 md:top-2.5" /><input className="input pl-9" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Nom, WhatsApp, email, wilaya…" /></label>
...
function FilterSelect({ label, value, onChange, children, disabled = false }: { label: string; value: string; onChange: (value: string) => void; children: React.ReactNode; disabled?: boolean }) { return <label className="w-full min-w-0 flex-1 sm:w-auto sm:min-w-48 sm:flex-none"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Filtrer par · {label}</span><select className="input w-full sm:w-52 disabled:opacity-50" value={value} disabled={disabled} onChange={(e) => onChange(e.target.value)}>{children}</select></label>; }
