...
import { CalendarClock, CheckCircle2, Copy, ExternalLink, ListChecks, Loader2, MessageCircle, Phone, Search, StickyNote, Trash2, Truck, UserCheck, Users, Video } from 'lucide-react';
...
export function AdminWebinarLeads() {
  ...
  const [statusTarget, setStatusTarget] = useState<WebinarLead | null>(null); const [statusDraft, setStatusDraft] = useState<WebinarLeadStatus>('to_call');
  const isCloser = !!profile && !profile.is_admin;
  // Quick views: closers land on "À appeler" (their pending work); admins on "Tous".
  const [quick, setQuick] = useState<'tous' | 'a_appeler' | 'rappels' | 'confirmes'>(isCloser ? 'a_appeler' : 'tous');
  const leads = leadsQ.data ?? [];
  const filtered = useMemo(() => { const needle = search.trim().toLowerCase(); const endToday = new Date(); endToday.setHours(23, 59, 59, 999); const endTodayMs = endToday.getTime(); const rows = leads.filter((lead) => {
    if (quick === 'a_appeler' && !['new', 'to_call', 'nrp', 'callback'].includes(lead.status)) return false;
    if (quick === 'confirmes' && !['confirmed', 'in_delivery', 'delivered'].includes(lead.status)) return false;
    ...
    return true;
  }); return [...rows].sort((a, b) => { if (sortBy === 'live') { const rank = (v: boolean | null | undefined) => v === true ? 2 : v === false ? 1 : 0; return rank(b.ready_to_pay) - rank(a.ready_to_pay); } if (sortBy === 'closer') return (a.closer_name ?? 'zzzz').localeCompare(b.closer_name ?? 'zzzz'); if (sortBy === 'crm') return a.status.localeCompare(b.status); if (sortBy === 'ecom') return (a.delivery_orders?.[0]?.ecom_situation ?? 'zzzz').localeCompare(b.delivery_orders?.[0]?.ecom_situation ?? 'zzzz'); return b.created_at.localeCompare(a.created_at); }); }, [leads, search, status, sortBy, quick]);
  const stats = useMemo(() => { const endToday = new Date(); endToday.setHours(23, 59, 59, 999); const endTodayMs = endToday.getTime(); return { total: leads.length, toCall: leads.filter((lead) => ['new', 'to_call', 'nrp', 'callback'].includes(lead.status)).length, confirmed: leads.filter((lead) => ['confirmed', 'in_delivery', 'delivered'].includes(lead.status)).length, rappels: leads.filter((lead) => lead.next_follow_up_at && new Date(lead.next_follow_up_at).getTime() <= endTodayMs).length }; }, [leads]);
  ...
  function openStatus(lead: WebinarLead) {
    ...
    const cur = BULK_STATUS_OPTIONS.some((o) => o.value === lead.status) ? lead.status as WebinarLeadStatus : 'to_call';
    setStatusDraft(cur);
  }
  async function saveStatus() {
    ...
    setSaving(true);
    try {
      await updateWebinarLeadStatus(statusTarget.id, statusDraft);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Kpi icon={Users} label={isCloser ? 'Mes prospects' : 'Prospects qualifiés'} value={stats.total} color="orange" /><Kpi icon={Phone} label="À appeler / rappeler" value={stats.toCall} color="orange" /><Kpi icon={CalendarClock} label="Rappels dus" value={stats.rappels} color={stats.rappels > 0 ? 'red' : 'orange'} /><Kpi icon={UserCheck} label={isCloser ? 'Mes ventes' : 'Commandes confirmées'} value={stats.confirmed} color="green" /></div>
    <section className="card overflow-hidden">
      <div className="flex flex-wrap items-center gap-2 border-b border-zinc-200 px-4 pt-4">
        {([['tous', `Tous (${leads.length})`], ['a_appeler', `À appeler (${stats.toCall})`], ['rappels', `Rappels dus (${stats.rappels})`], ['confirmes', `Confirmés (${stats.confirmed})`]] as const).map(([key, label]) => <button key={key} type="button" onClick={() => setQuick(key)} className={cn('rounded-full border px-4 py-1.5 text-sm font-medium transition', quick === key ? 'border-aurel-orange bg-aurel-orange/10 text-aurel-orange' : 'border-zinc-200 text-zinc-600 hover:border-zinc-300', key === 'rappels' && stats.rappels > 0 && quick !== 'rappels' && 'border-red-300 text-red-600')}>{label}</button>)}
      </div>
      <div className="flex flex-wrap gap-3 border-b border-zinc-200 p-4">
        ...
        <select className="input w-52 disabled:opacity-50" disabled={quick !== 'tous'} title={quick !== 'tous' ? 'Désactivé : une vue rapide est active' : undefined} value={status} onChange={(e) => setStatus(e.target.value as WebinarLeadStatus | 'all')}><option value="all">Tous les statuts</option>{FILTER_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
        <select className="input w-52" value={sortBy} onChange={(e) => setSortBy(e.target.value as typeof sortBy)}><option value="date">Trier par : date</option>{profile?.is_admin && <option value="closer">Trier par : closer</option>}<option value="crm">Trier par : statut CRM</option><option value="ecom">Trier par : statut E-com</option><option value="live">Trier par : prêt à payer</option></select>
    ...
    <Modal open={!!statusTarget} onClose={() => !saving && setStatusTarget(null)} title="Changer le statut" maxWidth="max-w-md">{statusTarget && <div className="space-y-4"><div className="rounded-card-sm bg-zinc-50 p-3 text-sm"><span className="font-semibold text-zinc-900">{statusTarget.full_name}</span> · <span className="text-zinc-500">{statusTarget.phone_raw}</span></div><label className="block"><span className="label">Nouveau statut</span><select className="input" value={statusDraft} onChange={(e) => setStatusDraft(e.target.value as WebinarLeadStatus)}>{BULK_STATUS_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><p className="text-xs text-zinc-500">Pour confirmer une vente (commande), utilise plutôt le bouton « Suivi ».</p><div className="flex justify-end gap-2"><button type="button" className="btn-outline" disabled={saving} onClick={() => setStatusTarget(null)}>Annuler</button><button type="button" className="btn-primary" disabled={saving} onClick={saveStatus}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <ListChecks className="h-4 w-4" />} Enregistrer</button></div></div>}</Modal>
