...
export function AdminWebinarLeads() {
  ...
  const [statusTarget, setStatusTarget] = useState<WebinarLead | null>(null); const [statusDraft, setStatusDraft] = useState<WebinarLeadStatus>('to_call'); const [statusNote, setStatusNote] = useState('');
  ...
  const filtered = useMemo(() => { const needle = search.trim().toLowerCase(); const endToday = new Date(); endToday.setHours(23, 59, 59, 999); const endTodayMs = endToday.getTime(); const baseRows = quick === 'anciens' ? historicLeads : currentLeads; const rows = baseRows.filter((lead) => {
    ...
    return true;
  }); return [...rows].sort((a, b) => { if (sortBy === 'live') { const rank = (v: boolean | null | undefined) => v === true ? 2 : v === false ? 1 : 0; return rank(b.ready_to_pay) - rank(a.ready_to_pay); } if (sortBy === 'closer') return (a.closer_name ?? 'zzzz').localeCompare(b.closer_name ?? 'zzzz'); if (sortBy === 'crm') return a.status.localeCompare(b.status); if (sortBy === 'ecom') return (a.delivery_orders?.[0]?.ecom_situation ?? 'zzzz').localeCompare(b.delivery_orders?.[0]?.ecom_situation ?? 'zzzz'); return b.created_at.localeCompare(a.created_at); }); }, [currentLeads, historicLeads, search, status, sortBy, quick]);
  const stats = useMemo(() => { const endToday = new Date(); endToday.setHours(23, 59, 59, 999); const endTodayMs = endToday.getTime(); return { total: currentLeads.length, toCall: currentLeads.filter((lead) => ['new', 'to_call', 'nrp', 'callback'].includes(lead.status)).length, confirmed: currentLeads.filter((lead) => ['confirmed', 'in_delivery'].includes(lead.status)).length, delivered: currentLeads.filter((lead) => lead.status === 'delivered').length, rappels: currentLeads.filter((lead) => lead.next_follow_up_at && new Date(lead.next_follow_up_at).getTime() <= endTodayMs).length }; }, [currentLeads]);
  ...
  function openStatus(lead: WebinarLead) {
    ...
    const available = isCloser ? CLOSER_STATUS_OPTIONS : ADMIN_STATUS_OPTIONS;
  ...
  async function saveStatus() {
    ...
    setSaving(true);
    try {
      await updateWebinarLeadStatus(statusTarget.id, statusDraft, statusNote);
  ...
  const quickOptions: { key: typeof quick; label: string }[] = [
    { key: 'tous', label: `Tous (${currentLeads.length})` },
    ...
    ...(!isCloser ? [{ key: 'anciens' as const, label: `Anciens (${historicLeads.length})` }] : []),
  ];
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Kpi icon={Users} label={isCloser ? 'Mes prospects' : 'Prospects qualifiés'} value={stats.total} color="orange" /><Kpi icon={Phone} label="À appeler / rappeler" value={stats.toCall} color="orange" /><Kpi icon={CalendarClock} label="Rappels dus" value={stats.rappels} color={stats.rappels > 0 ? 'red' : 'orange'} /><Kpi icon={UserCheck} label={isCloser ? 'Mes ventes livrées' : 'Ventes livrées'} value={stats.delivered} color="green" /></div>
    ...
    <Modal open={!!statusTarget} onClose={() => !saving && setStatusTarget(null)} title="Changer le statut" maxWidth="max-w-md">{statusTarget && <div className="space-y-4"><div className="rounded-card-sm bg-zinc-50 p-3 text-sm"><span className="font-semibold text-zinc-900">{statusTarget.full_name}</span> · <span className="text-zinc-500">{statusTarget.phone_raw}</span></div><label className="block"><span className="label">Nouveau statut</span><select className="input" value={statusDraft} onChange={(e) => setStatusDraft(e.target.value as WebinarLeadStatus)}>{(isCloser ? CLOSER_STATUS_OPTIONS : ADMIN_STATUS_OPTIONS).map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label className="block"><span className="label">Note du changement (optionnelle)</span><textarea className="input min-h-24 resize-y" maxLength={2000} value={statusNote} onChange={(e) => setStatusNote(e.target.value)} placeholder="Pourquoi ce statut change ? Cette note sera visible par les admins." /></label><p className="text-xs text-zinc-500">{isCloser ? '« Confirmé » indique un colis confirmé, pas une vente. Seul un admin peut valider « Livré » et compter la vente.' : 'Une vente est comptée uniquement lorsque le statut devient « Livré ».'}</p><div className="flex justify-end gap-2"><button type="button" className="btn-outline" disabled={saving} onClick={() => setStatusTarget(null)}>Annuler</button><button type="button" className="btn-primary" disabled={saving} onClick={saveStatus}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <ListChecks className="h-4 w-4" />} Enregistrer</button></div></div>}</Modal>
