...
export function AdminWebinarLeads() {
  ...
  const invalidRegistrationRange = !!registrationFrom && !!registrationTo && registrationFrom > registrationTo;
  ...
  const detailedHistory = !!profile?.is_admin && !closerExperience;
  ...
  const filtered = useMemo(() => { const needle = search.trim().toLowerCase(); const rows = scopedLeads.filter((lead) => {
    if (!matchesRegistrationDateRange(lead.created_at, registrationFrom, registrationTo)) return false;
    ...
    return true;
  }); return [...rows].sort((a, b) => { if (sortBy === 'live') { const rank = (v: boolean | null | undefined) => v === true ? 2 : v === false ? 1 : 0; return rank(b.ready_to_pay) - rank(a.ready_to_pay); } if (sortBy === 'closer') return (a.closer_name ?? 'zzzz').localeCompare(b.closer_name ?? 'zzzz'); if (sortBy === 'crm') return a.status.localeCompare(b.status); if (sortBy === 'ecom') return (a.delivery_orders?.[0]?.ecom_situation ?? 'zzzz').localeCompare(b.delivery_orders?.[0]?.ecom_situation ?? 'zzzz'); return compareRegistrationDate(a, b, sortBy); }); }, [scopedLeads, search, status, sortBy, adminViewCloser, closerFilter, ecomFilter, readyFilter, registrationFrom, registrationTo]);
  ...
  const historyQ = useQuery({
    queryKey: detailedHistory ? queryKeys.webinarLeadAdminHistory(historyTarget?.id ?? '') : queryKeys.webinarLeadHistory(historyTarget?.id ?? ''),
    queryFn: () => detailedHistory ? fetchWebinarLeadAdminHistory(historyTarget!.id) : fetchWebinarLeadHistory(historyTarget!.id),
    enabled: !!historyTarget,
    refetchInterval: historyTarget ? 5_000 : false,
  });
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'webinar_lead_admin_events' }, () => {
        if (profile.is_admin && historyTarget?.id) void qc.invalidateQueries({ queryKey: queryKeys.webinarLeadAdminHistory(historyTarget.id) });
      })
      .subscribe();
    return () => { void supabase.removeChannel(channel); };
  }, [historyTarget?.id, profile?.id, qc]);

  function recordContact(lead: WebinarLead, channel: 'phone' | 'whatsapp') {
    setContactTarget(null);
    // Keep the native link immediate; logging must not break app handoff/popups.
    void logWebinarContact(lead.id, channel)
      .then(() => qc.invalidateQueries({ queryKey: queryKeys.webinarLeadAdminHistory(lead.id) }))
      .catch(() => toast.error('Le clic de contact n’a pas pu être enregistré dans l’historique.'));
  }
  ...
  return <div className="space-y-6">
    ...
    <section className="card overflow-hidden">
      <div className="border-b border-zinc-200 p-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <FilterSelect label="Ordre d’affichage" value={sortBy} onChange={(value) => setSortBy(value as typeof sortBy)}><option value="date_desc">Plus récents d’abord</option><option value="date_asc">Plus anciens d’abord</option>{profile?.is_admin && <option value="closer">Closer : A → Z</option>}<option value="crm">Statut CRM</option><option value="ecom">Statut E-com</option><option value="live">Prêt à payer</option></FilterSelect>
          <fieldset className="min-w-0 rounded-lg border border-zinc-200 p-3 sm:col-span-2 lg:col-span-3 xl:col-span-6">
            <legend className="px-1 text-sm font-semibold text-zinc-700">Filtrer par date d’inscription</legend>
            <div className="grid grid-cols-1 items-end gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto]">
              <label className="min-w-0"><span className="label">Du (date de début)</span>
                <input type="date" className="input min-h-[44px] w-full min-w-0" value={registrationFrom}
                  max={registrationTo || undefined} aria-invalid={invalidRegistrationRange} aria-describedby="registration-date-help"
                  onChange={(e) => { setRegistrationFrom(e.target.value); setSelectedIds(new Set()); }} />
              </label>
              <label className="min-w-0"><span className="label">Au (date de fin)</span>
                <input type="date" className="input min-h-[44px] w-full min-w-0" value={registrationTo}
                  min={registrationFrom || undefined} aria-invalid={invalidRegistrationRange} aria-describedby="registration-date-help"
                  onChange={(e) => { setRegistrationTo(e.target.value); setSelectedIds(new Set()); }} />
              </label>
              <button type="button" className="btn-outline min-h-[44px]" disabled={!registrationFrom && !registrationTo}
                onClick={() => { setRegistrationFrom(''); setRegistrationTo(''); setSelectedIds(new Set()); }}>Effacer les dates</button>
            </div>
            <p id="registration-date-help" className={cn('mt-2 text-xs', invalidRegistrationRange ? 'text-red-600' : 'text-zinc-500')}>
              {invalidRegistrationRange ? 'La date de fin doit être égale ou postérieure à la date de début.' : 'Dates incluses · Heure d’Alger. Pour un seul jour, choisis la même date dans les deux calendriers.'}
            </p>
          </fieldset>
        </div>
      ...
      <div className="border-b border-zinc-200 bg-zinc-50 px-4 py-2 text-xs text-zinc-600"><strong>{filtered.length}</strong> prospect(s) affiché(s) · Vue : {isCloser ? 'closer' : adminViewCloser ? `Closer ${adminViewCloser}` : 'administrateur'} · CRM : {status === 'all' ? 'tous' : STATUS_LABEL[status]} · Closer : {closerFilter === '__unassigned' ? 'non attribués' : closerFilter || 'tous'} · E-com : {ecomFilter === '__none' ? 'sans statut' : ecomFilter || 'tous'} · Inscription : {registrationFrom || registrationTo ? `${registrationFrom ? registrationFrom.split('-').reverse().join('/') : 'début'} → ${registrationTo ? registrationTo.split('-').reverse().join('/') : 'sans limite'}` : 'toutes les dates'} · Ordre : {sortBy === 'date_desc' ? 'plus récent' : sortBy === 'date_asc' ? 'plus ancien' : sortBy.replace('_', ' ')}</div>
    ...
    <Modal open={!!historyTarget} onClose={() => setHistoryTarget(null)} title={`Historique${detailedHistory ? ' détaillé · Admin' : ''}${historyTarget ? ` — ${historyTarget.full_name}` : ''}`} maxWidth="max-w-2xl"><LeadHistoryTimeline detailed={detailedHistory} loading={historyQ.isLoading} error={historyQ.isError} activities={historyQ.data ?? []} /></Modal>
    <Modal open={!!contactTarget} onClose={() => setContactTarget(null)} title={`Appeler${contactTarget ? ` — ${contactTarget.full_name}` : ''}`} maxWidth="max-w-md">{contactTarget && <div className="space-y-4"><p className="text-sm text-zinc-600">Choisis comment contacter ce prospect.</p><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><a className="btn-primary justify-center py-3" href={whatsappUrl(contactTarget.phone_normalized)} target="_blank" rel="noreferrer" onClick={() => recordContact(contactTarget, 'whatsapp')}><MessageCircle className="h-5 w-5" /> Appel WhatsApp</a><a className="btn-outline justify-center py-3" href={`tel:${internationalPhone(contactTarget.phone_normalized)}`} onClick={() => recordContact(contactTarget, 'phone')}><Phone className="h-5 w-5" /> Appel téléphonique</a></div><p className="text-center text-xs text-zinc-400">{contactTarget.phone_raw}</p></div>}</Modal>
...
function LeadRow({ lead, selected, onToggle, onContact, onWhatsApp, onCall, onStatus, onHistory, onDelete, onAssign }: { lead: WebinarLead; selected: boolean; onToggle: () => void; onContact: () => void; onWhatsApp: () => void; onCall?: () => void; onStatus: () => void; onHistory: () => void; onDelete?: () => void; onAssign?: () => void }) { const order = lead.delivery_orders?.[0]; return <tr className="align-top hover:bg-zinc-50/70"><td className="px-3 py-3"><input type="checkbox" className="h-6 w-6 cursor-pointer rounded border-2 border-zinc-400 accent-aurel-orange" aria-label={`Sélectionner ${lead.full_name}`} checked={selected} onChange={onToggle} /></td><td className="px-4 py-3"><div className="font-semibold text-zinc-900">{lead.full_name}</div><a className="text-xs text-aurel-teal hover:underline" href={whatsappUrl(lead.phone_normalized)} target="_blank" rel="noreferrer" onClick={onWhatsApp}>{lead.phone_raw}</a><div className="text-[11px] text-zinc-400">{lead.email}</div>{lead.latest_call_note && <div className="mt-1 max-w-56 rounded bg-amber-50 px-2 py-1 text-amber-800"><StickyNote className="mr-1 inline h-3 w-3" />{lead.latest_call_note}</div>}</td><td className="px-4 py-3">{lead.ready_to_pay == null ? <span className="text-xs text-zinc-400" title="Question posée uniquement depuis le nouveau formulaire">—</span> : <span className={cn('badge', lead.ready_to_pay ? 'badge-green' : 'badge-slate')}>{lead.ready_to_pay ? 'Oui' : 'Non'}</span>}</td><td className="px-4 py-3"><MapPinText lead={lead} /></td><td className="px-4 py-3">{lead.closer_name ?? '—'}<div className="text-[10px] text-zinc-400">{lead.call_count} appel(s)</div>{lead.next_follow_up_at && <div className="mt-1 text-aurel-orange">Rappel prévu : <CalendarClock className="mr-1 inline h-3 w-3" />{formatDateTime(lead.next_follow_up_at)}</div>}</td><td className="px-4 py-3"><LeadStatus status={lead.status} /></td><td className="px-4 py-3">{order ? <><span className="badge badge-teal">{order.ecom_situation || (order.ecom_tracking ? 'En préparation' : 'Pas encore envoyée')}</span>{order.ecom_tracking && <div className="mt-1 font-mono text-[10px] text-zinc-500">{order.ecom_tracking}</div>}</> : <span className="text-xs text-zinc-400">Aucune commande</span>}</td><td className="px-4 py-3 text-xs text-zinc-500"><time dateTime={lead.created_at}>{formatRegistrationDate(lead.created_at)}</time></td><td className="px-4 py-3"><div className="flex flex-wrap justify-end gap-1.5">{onAssign && <button type="button" className="btn-outline px-2.5 py-1.5 text-xs" onClick={onAssign}><UserCheck className="h-3.5 w-3.5" /> Attribuer</button>}<button type="button" className="btn-outline px-2.5 py-1.5 text-xs" onClick={onContact}><Phone className="h-3.5 w-3.5" /> Appeler</button>{onCall && <button type="button" className="btn-outline px-2.5 py-1.5 text-xs" onClick={onCall}><Phone className="h-3.5 w-3.5" /> Suivi</button>}<button type="button" className="btn-outline px-2.5 py-1.5 text-xs" onClick={onStatus}><ListChecks className="h-3.5 w-3.5" /> Statut</button><button type="button" className="btn-outline px-2.5 py-1.5 text-xs" onClick={onHistory}><History className="h-3.5 w-3.5" /> Historique</button>{onDelete && <button type="button" aria-label="Supprimer le prospect" className="btn-outline px-2.5 py-1.5 text-red-600" onClick={onDelete}><Trash2 className="h-3.5 w-3.5" /></button>}</div></td></tr>; }

type LeadActions = { lead: WebinarLead; selected: boolean; onToggle: () => void; onContact: () => void; onWhatsApp: () => void; onCall?: () => void; onStatus: () => void; onHistory: () => void; onDelete?: () => void; onAssign?: () => void };
function LeadCard({ lead, selected, onToggle, onContact, onWhatsApp, onCall, onStatus, onHistory, onDelete, onAssign }: LeadActions) {
  ...
  return <article className="space-y-3 p-4">
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1"><div className="break-words font-semibold text-zinc-950">{lead.full_name}</div><a className="mt-0.5 inline-block text-sm font-medium text-aurel-teal" href={whatsappUrl(lead.phone_normalized)} target="_blank" rel="noreferrer" onClick={onWhatsApp}>{lead.phone_raw}</a><div className="break-all text-xs text-zinc-500">{lead.email}</div></div>
    ...
    <div className="grid grid-cols-2 gap-3 rounded-lg bg-zinc-50 p-3 text-xs">
      ...
      <div><div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Date d’inscription</div><div className="text-zinc-600"><time dateTime={lead.created_at}>{formatRegistrationDate(lead.created_at)}</time></div></div>
      {lead.next_follow_up_at && <div className="col-span-2"><div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Rappel prévu</div><div className="text-aurel-orange"><CalendarClock className="mr-1 inline h-3.5 w-3.5" />{formatDateTime(lead.next_follow_up_at)}</div></div>}
    </div>
...
function LeadHistoryTimeline({ loading, error, activities, detailed = false }: { loading: boolean; error: boolean; activities: WebinarLeadActivity[]; detailed?: boolean }) {
  ...
  return <div className="max-h-[65vh] space-y-3 overflow-y-auto pr-1">
    ...
    {activities.map((activity) => {
      ...
      const title = detailed ? adminActivityTitle(activity) : activity.status ? STATUS_LABEL[activity.status] : activity.activity_type === 'delivery' ? 'Commande' : 'Mise à jour';
      const actorRole = { admin: 'Admin', closer: 'Closer', system: 'Système', unknown: 'Rôle non enregistré' }[activity.actor_role ?? 'unknown'];
      return <article key={activity.id} className="relative rounded-card-sm border border-zinc-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            <span className={cn('badge', !detailed && activity.status ? leadStatusColor(activity.status) : 'badge-slate')}>{title}</span>
            ...
            {detailed && <span className="badge badge-slate">{actorRole}</span>}
          </div>
          <time dateTime={activity.created_at} className="text-xs text-zinc-500">{detailed ? formatAuditDate(activity.created_at) : formatDateTime(activity.created_at)}</time>
        </div>
        {detailed && activity.status && <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
          {activity.previous_status && <><LeadStatus status={activity.previous_status} /><span aria-label="vers">→</span></>}
          <LeadStatus status={activity.status} />
        </div>}
        {detailed && activity.activity_type === 'assignment' && <p className="mt-2 break-words text-sm text-zinc-700">
          {typeof metadata.previous_closer_name === 'string' && metadata.previous_closer_name ? metadata.previous_closer_name : 'Non attribué'} → {typeof metadata.closer_name === 'string' && metadata.closer_name ? metadata.closer_name : 'Non attribué'}
        </p>}
        {detailed && typeof metadata.closer_name === 'string' && activity.activity_type === 'call' && <p className="mt-2 text-xs text-zinc-600">Suivi déclaré pour : {metadata.closer_name}</p>}
        {detailed && typeof metadata.external_reference === 'string' && <p className="mt-2 break-all text-xs text-zinc-600">Référence commande : <strong>{metadata.external_reference}</strong></p>}
        {detailed && typeof metadata.delivery_order_id === 'string' && <p className="mt-1 break-all font-mono text-[10px] text-zinc-500">ID commande : {metadata.delivery_order_id}</p>}
        {detailed && activity.activity_type === 'contact' && <p className="mt-2 text-xs text-zinc-600">Ouverture du lien uniquement ; résultat de l’appel non vérifié.</p>}
        {detailed && metadata.audit_legacy === true && <p className="mt-2 text-xs text-zinc-500">Archive : uniquement les informations enregistrées à l’époque. Les détails manquants ne sont pas supposés.</p>}
        ...
        {followUp && <div className="mt-2 text-xs font-medium text-aurel-orange"><CalendarClock className="mr-1 inline h-3.5 w-3.5" />Rappel prévu : {detailed ? formatAuditDate(followUp) : formatDateTime(followUp)}</div>}
      </article>;
    })}
  </div>;
}
...
function LeadStatus({ status }: { status: WebinarLeadStatus }) { return <span className={cn('badge', leadStatusColor(status))}>{STATUS_LABEL[status]}</span>; }
