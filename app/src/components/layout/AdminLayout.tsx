...
import {
  ...
  MessageCircle, Truck, ClipboardList, UserRoundCheck, FilePenLine, LockKeyhole, Trophy, Filter, PackageCheck, Gauge,
} from 'lucide-react';
...
const NAV_GROUPS: { label: string; items: { to: string; label: string; icon: typeof LayoutDashboard; end?: boolean; alertKey?: 'pending' }[] }[] = [
  {
    ...
    items: [
      ...
      { to: '/admin/funnel',    label: 'Tunnel complet',  icon: Filter },
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/admin/cod-health',   label: 'Santé COD',    icon: PackageCheck },
      ...
      { to: '/admin/readiness', label: 'Readiness simulator', icon: Gauge },
